...
async function refreshLiveCache(env, ctx) {
  try {
    ...
    const dataPromises = rawEvents.map(async (event) => {
      ...
      let homePen = null;
      let awayPen = null;
      let phase = event.isHt ? "HT" : event.status === "live" ? "Live" : "FT";
...
// .wrangler/tmp/bundle-WeubaQ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-WeubaQ/middleware-loader.entry.ts
